...
export const API_CONFIG = {
  ...
  COMPUTER_IP: "10.130.198.121", // Updated to match ipconfig Wi-Fi IPv4

  // API Base URLs
  LOCALHOST: "http://localhost:3000/api",
  NETWORK: "http://10.130.198.121:3000/api", // Updated to match ipconfig Wi-Fi IPv4
  ...
  AUTO_DETECT: false, // Force configured NETWORK IP for same Wi-Fi/LAN devices
};
...
const buildUrlFromHost = (hostWithPort: string) =>
  `http://${hostWithPort}${API_PATH}`;
const isLoopbackHost = (hostWithPort: string) => {
...
export const getApiUrl = () => {
  ...
  if (__DEV__) {
    ...
    console.log(
      "[API CONFIG] detectedHost via SourceModule:",
      getSourceModuleHost(),
    );
  ...
  if (API_CONFIG.AUTO_DETECT && detectedHost && !isLoopbackHost(detectedHost)) {
  ...
  const fallbackUrl =
    API_CONFIG.ENVIRONMENT === "network"
      ? API_CONFIG.NETWORK
  ...
  if (__DEV__) {
    ...
    if (API_CONFIG.AUTO_DETECT && !detectedHost) {
      console.warn(
        "[API CONFIG] ⚠️ AUTO_DETECT is enabled but no host detected. Using configured IP.",
      );
    }
  }
